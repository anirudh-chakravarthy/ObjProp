...
def bbox_target(pos_bboxes_list,
                ...
                concat=True):
    labels, label_weights, bbox_targets, bbox_weights, ids, id_weights = multi_apply(
        bbox_target_single,
        pos_bboxes_list,
        neg_bboxes_list,
        pos_gt_bboxes_list,
    ...
    return (labels, label_weights, bbox_targets, bbox_weights), (ids, id_weights)
...
def bbox_target_single(pos_bboxes,
    ...
    ids = pos_bboxes.new_zeros(num_samples, dtype=torch.long)
    label_weights = pos_bboxes.new_zeros(num_samples)
    id_weights = pos_bboxes.new_zeros(num_samples)
    bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
    bbox_weights = pos_bboxes.new_zeros(num_samples, 4)
    if num_pos > 0:
        labels[:num_pos] = pos_gt_labels
        ids[:num_pos] = pos_gt_ids
