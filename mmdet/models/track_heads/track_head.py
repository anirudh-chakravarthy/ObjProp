...
class TrackHead(nn.Module):
    ...
    @auto_fp16()
    def forward(self, x, ref_x, x_n, ref_x_n, test_mode=False):
        ...
        # x = self.get_track_feats(x)
        # ref_x = self.get_track_feats(ref_x)
        if test_mode:
            x = F.normalize(self.get_track_feats(x))
            ref_x = F.normalize(self.get_track_feats(ref_x))
        else:
            x = self.get_track_feats(x)
            ref_x = self.get_track_feats(ref_x)
        # x = x.view(x.size(0), -1)
    ...
    def loss(self,
        ...
        if self.dynamic:
            ...
            for score, cur_ids, cur_weights in zip(match_score, ids, id_weights):
                ...
                n_total += n_valid
                loss_match += self.loss_match(score, cur_ids, weight=cur_weights)
            ...
            if n_total > 0:
                ...
                losses['loss_match'] = losses['loss_match'].reshape(1)
            else:
                losses['match_acc'] = torch.tensor(match_acc).reshape(1).to(
                    torch.cuda.current_device())
                losses['loss_match'] = torch.tensor(losses['loss_match']).reshape(1).to(
                    torch.cuda.current_device())
        else:
          if match_score is not None:
              valid_idx = torch.nonzero(cur_weights).squeeze()
              losses['loss_match'] = self.loss_match(match_score, ids, weight=id_weights)
